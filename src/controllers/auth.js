...
import * as zoomApi from "../utils/zoom-api.js";
import * as zoomHelpers from "../utils/zoom-helpers.js";
import * as store from "../utils/store.js";

// In-client OAuth 1/2
export const inClientAuthorize = async (req, res, next) => {
  console.log(
...
// In-Client OAuth 2/2
export const inClientOnAuthorized = async (req, res, next) => {
  console.log(
...
export const proxy = createProxyMiddleware({
  ...
  onProxyReq: (proxyReq, req, res) => {
    ...
    // Forward important Zoom headers to frontend
    if (req.headers["x-zoom-app-context"]) {
      proxyReq.setHeader(
        "x-zoom-app-context",
        req.headers["x-zoom-app-context"]
    ...
    // Add session info as headers for frontend
    if (req.session.user) {
      proxyReq.setHeader("X-Zoom-User-ID", req.session.user);
    }
    if (req.session.meetingUUID) {
      proxyReq.setHeader("X-Zoom-Meeting-UUID", req.session.meetingUUID);
    }
