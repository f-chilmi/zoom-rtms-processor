import { refreshZoomAccessToken } from "../utils/zoom-api.js";
...
export const getUser = async (req, res, next) => {
  ...
  try {
    ...
    return next(new Error("Error getting user from session: " + error.message));
  }
};
...
export const setZoomAuthHeader = async (req, res, next) => {
  try {
    if (!req.session.user) {
      throw new Error(
